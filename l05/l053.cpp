...
auto UnRAllAngle = vector<vector<int>>();
//saves points info
class Point {
...
void algo(int row, int col, int high, int low, string output, string both, string hys) {
    ...
    for (int g = 0; g < col; g++) {
        UnRAllAngle.push_back(vector<int>(row, 0));
    }
    ...
    for (int i = 0; i < col; i++) {
        for (int j = 0; j < row; j++) {
            ...
            UnRAllAngle[i][j] = comparingAngle;
...
void afterCannyLinesNCircle(int highthres, int lowerthres, string filename, string nms, string grey, string fin, string outfile, string www) { // only call after doing nms and cannyedge, use temp AFTER algo() called
    ...
    for (int i = 0; i < y10; i++) { //col
        for (int j = 0; j < x10; j++) { //row
            if (temp[i][j] == 1) {
                //lm += 1;
                //if (lm % 8 == 0) {
                int x8 = j + (int)(x10 * cos((UnRAllAngle[i][j] * 180 / 3.14159)));
                int y8 = i + (int)(y10 * sin((UnRAllAngle[i][j] * 180 / 3.14159)));
                totalbresenham(Line(Point(j, i), Point(x8, y8)));
                // }
            }
        }
    }
    ...
    createppm(x10, y10, 1, www, bresy, true);
    ...
    testcolor(x10, y10, 255, "normal", bresy);
...
int main(int argc, char** argv)
{
    ...
    string outfile, filename, grey, nms, fin, www;
    lowerthres = 200;
    highthres = 400;
    outfile = "image1.ppm";
    nms = "image2.ppm";
    ...
    afterCannyLinesNCircle(highthres, lowerthres, filename, nms, grey, fin, outfile, www);
}