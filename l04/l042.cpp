...
public:     // available member methods
    ...
    static list<Point> getSortedy(list<Point> listOfPoints) {
        listOfPoints.sort(sortFuncty); // invoke system sort with sortFunct comparator
        return listOfPoints;
    }
    ~Point() {
...
private:
    ...
    static bool sortFuncty(Point i, Point j) { return (j.y > i.y); }; // compare by y-value
};
...
//makes a ppm file
void createppm(bool name) {
    ofstream file, file3;
    if (name == true) {
        file.open("grahamscan.ppm");
    }
    else {
...
void partone() {
    generate();
    createarr(l, f);
    list<Point> listy = Point::getSorted(getpoints());
    vector<Point> sixty{ std::begin(listy), std::end(listy) };
    vector<Point>& six = sixty;
    QuickHull(six);
    createppm(false);
}
bool NotALeftTurn(Point A, Point B, Point C) {
    double posneg = (B.getX() - A.getX()) * (C.getY() - B.getY()) - (B.getY() - A.getY()) * (C.getX() - B.getX());
    //double posneg1 = 
    if (posneg < 0) {
        return true;
    }
...
//yLine = (minX.getX() - maxX.getX()) * (sixty[i].getY() - maxX.getY()) - (minX.getY() - maxX.getY()) * (sixty[i].getX() - maxX.getX()); 
//if this number is less than 0 its a left turn from minx to maxx
Point p0;
bool comparetheta(Point p, Point p2) {
    double cross_product = ((p.getX() - p0.getX()) * (p2.getY() - p0.getY())) - ((p.getY() - p0.getY()) * (p2.getX() - p0.getX()));
    return cross_product > 0;
}
void parttwo() {
    //read file from points.txt same thin as 3.4
  //  generate(); //puts 60 points in a file
    list<Point> listy = Point::getSortedy(getpoints()); //reads file
    createarr(l, f);
    vector<Point> sixty{ std::begin(listy), std::end(listy) };
    for (Point e : sixty) {
        illuminate(e.getX()*l, e.getY()*f);
        circle(2, e);
        circle(3, e);
    }
    ...
    double count2 = 0;
    while (count >= 0) {
        if (sixty[count+1].getY() == sixty[count].getY()) {
            if (sixty[count + 1].getX() < sixty[count].getX()) {
                count++;
                count2 = count;
                p0 = sixty[count2];   
            }
    ...
    sixty.erase(sixty.begin() + count2);
    sort(sixty.begin(), sixty.end(), comparetheta);
    cout << "this is p0 " << p0.getX() << " " << p0.getY() << endl;
   // circle(10, p0);
    //for (Point e : sixty) {
    ...
    stackPoints.push(p0);
    stackPoints.push(p1);
    for (int i = 1; i < sixty.size(); i++) {
        Point B = stackPoints.top();
        stackPoints.pop();
        Point A = stackPoints.top();
        while ((stackPoints.size() > 1) && (NotALeftTurn(A, B, sixty[i]))) {
            B = stackPoints.top();
            stackPoints.pop();
            A = stackPoints.top();
        }
        stackPoints.push(B);
        stackPoints.push(sixty[i]);
    }
    //for (int i = 0; i < stackPoints.size(); i++) {
    //    circle(6, stackPoints.top());
    //    cout << stackPoints.top().getX() << " " << stackPoints.top().getY() << endl;
    //    stackPoints.pop();
    //}
    vector<Point> vectorPoints;
   // vectorPoints.push_back(p0);
    while (stackPoints.size() > 0)
    {
        vectorPoints.push_back(stackPoints.top());
        stackPoints.pop();
    }
    vectorPoints.push_back(p0);
    totalbresenham(vectorPoints[vectorPoints.size() - 1], vectorPoints[0]);
    for (int i = 0; i < vectorPoints.size() - 1; i++) {
        totalbresenham(vectorPoints[i], vectorPoints[i + 1]);
    }
    //2 ppm matches uses random points.txt
    //also do targeted testing, that tests certain points.txt share a points.txt w someone
    createppm(true);
}
int main()
{
