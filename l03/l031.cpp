...
void partzero() {
    ...
    cout << "you wanna generate random points bro? (enter yes for Yes and no for No)" << endl;
    cin >> x;
    createarr(p, b);
...
void partone() { //fix opening before friday, do brute force in class/weeknd
    ...
    while (getline(file1, temp)) {
        ...
        l.push_back(Point(stof(temp.substr(0, pos)), stof(temp.substr(pos + 1, temp.length()))));
    }
    file1.close();
    for (Point x: l) {
